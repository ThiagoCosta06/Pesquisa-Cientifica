...
for da in da_list:
    ...
    cmd_str = f'nohup python CNNpestes.py --ds USK-Coffee --arch alexnet --optim none ' + \
              f'--bs 16 --lr 0.00001 --ep {epochs} --optimizer Adam --ft --da {da} --ec {ec} --es ' + \
              f'--patience 21 --num_workers 0'
    ec = ec + 1
    os.system(cmd_str)

    cmd_str = f'nohup python CNNpestes.py --ds USK-Coffee --arch efficientnet_b4 --optim none ' + \
              f'--bs 16 --lr 0.001 --ep {epochs} --optimizer Adam --ft --da {da} --ec {ec} --es ' + \
              f'--patience 21 --num_workers 0'
    ec = ec + 1
    os.system(cmd_str)

    # cmd_str = f'nohup python train_test.py --ds USK-Coffee --arch resnet50 --optim none ' + \
    #           f'--bs 64 --lr 0.0001 --ep {epochs} --optimizer Adam --ft --da {da} --ec {ec} --es ' + \
